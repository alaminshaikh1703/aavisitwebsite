  ...
  return (
    <section className="py-24 bg-white relative border-b border-slate-100">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-6xl">
        ...
        {/* Tab Navigation */}
        <div className="flex flex-wrap justify-center gap-2 mb-12 bg-slate-100 p-2 rounded-2xl border border-slate-200 w-fit mx-auto">
          {categories.map((cat) => {
            ...
            return (
              ...
              >
                {isActive && (
                  <motion.div
                    layoutId="activeTab"
                    className="absolute inset-0 bg-slate-200 border border-slate-300 rounded-xl"
        ...
        {/* Tab Content */}
        <div className="bg-slate-50 border border-slate-200 rounded-3xl p-8 lg:p-12 shadow-2xl relative overflow-hidden">
           ...
           <AnimatePresence mode="wait">
             ...
             >
               <div>
                  <h4 className="text-3xl font-black text-slate-900 mb-4">{activeCategory.title}</h4>
                  ...
                  <Link 
                     href="/contact"
                     className="inline-flex items-center text-[#00B7FF] font-bold hover:text-slate-900 transition-colors group"
                  >
                     Discuss {activeCategory.name} <ArrowRight className="ml-2 w-4 h-4 group-hover:translate-x-1 transition-transform" />
                  </Link>
               </div>

               <div className="relative aspect-video rounded-2xl overflow-hidden border border-slate-200 shadow-2xl group">
                  ...
                  <div className="absolute bottom-4 left-4 right-4 z-20 flex items-center justify-between">
                     <span className="px-3 py-1 bg-slate-200 backdrop-blur border border-slate-300 rounded-full text-xs font-bold text-slate-900 uppercase tracking-wider">
                        Live Preview
                     </span>
                  </div>
               </div>
